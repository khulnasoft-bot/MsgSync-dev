...
async function getReports(req, res) {
    try {
        ...
        const data = await analyticsService.getDetailedReports(filters, parseInt(skip) || 0, parseInt(take) || 50);
...
async function getAlerts(req, res) {
    try {
        const organizationId = req.organization ? req.organization.id : null;
        if (!organizationId) return res.status(403).json({ status: 'error', message: 'Unauthorized' });
