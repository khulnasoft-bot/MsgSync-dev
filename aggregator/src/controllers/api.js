...
async function triggerAggregation(req, res) {
    try {
        ...
        res.status(200).json({ status: 'success', message: 'Aggregation triggered successfully' });
    } catch (error) {
...
async function addSource(req, res) {
    ...
    if (!name || !type || !config) {
        return res.status(400).json({ status: 'error', message: 'Missing required fields: name, type, config' });
    }
