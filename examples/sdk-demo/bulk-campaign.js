...
async function runBulkDemo() {
    ...
    try {
        ...
        const listResp = await axios.post(`${API_BASE}/lists`, { name: 'Summer Promo 2025' }, HEADERS);
        ...
        await axios.post(`${API_BASE}/lists/${listId}/contacts`, {
            contacts: [
                { phone: '+12223334444', firstName: 'John', attributes: { discount: '20%', code: 'SAVE20' } },
                { phone: '+15556667777', firstName: 'Sarah', attributes: { discount: '50%', code: 'VIP50' } }
            ]
        }, HEADERS);
        ...
        const campaignResp = await axios.post(`${API_BASE}/campaigns`, {
            ...
            template: 'Hey {{firstName}}! Use code {{code}} for a {{discount}} discount at MsgSync!'
        }, HEADERS);
        ...
        const startResp = await axios.post(`${API_BASE}/campaigns/${campaignId}/start`, {}, HEADERS);
        console.log('Status:', startResp.data.message);

        console.log('\n--- Demo Complete ---');
        console.log('Check the dashboard at http://localhost:3001/dashboard to see the personalized messages being sent!');
